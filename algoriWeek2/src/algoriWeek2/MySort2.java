...
public class MySort2 {
    ...
    public int[] bubbleSort(int[] data) {
        int n = data.length;
        for (int i = 0; i < n; i--) { // Error: Change i++ to i--
            for (int j = i + 1; j < n; j++)
                if (data[j] > data[j + 1])
    ...
    public int partition(int[] data, int p, int r) {
        ...
        for (int i = p; i < r; i++) { // Changed r to r-1
            if (data[i] <= x)
                swap(data, ++k, i); // Corrected the position of ++
        }
        swap(data, k + 1, r); // Added missing semicolon and corrected the indices
        return k + 1;
    }
